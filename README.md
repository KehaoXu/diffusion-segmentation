...
- Mixed real ATLAS-style MRI cases with synthetic samples generated by the `USB/` pipeline.
- Evaluated synthetic-to-real ratios from `0.0` to `0.9` across existing split and generation seeds.
- Aggregated 69 completed evaluation runs and generated result tables and plots.
- Refactored split CSV handling into reusable utilities with tests and compact, portable path storage.
...
+-- USB/                             # Synthetic-data generation code
```

`USB/` contains synthetic-data generation code from [USB](https://github.com/jhuldr/USB.git). The segmentation workflow consumes synthetic samples through `--gen-root`.
...
- Current results are based on existing ATLAS-style experiments.
